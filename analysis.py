...
results_path = os.path.join(od_path, "Work\\P_curve_shape\\dat\\test")
...
# f = [k for k in f if "Qrev0.00" in k]
...
for i, file in enumerate(f[:]):
    ...
    if not data_fits.empty and not overwrite and runName in data_fits.runName.to_list():
        ...
        if plot_curves:
            ...
            label = f"{runName}_(R2{round(R2, 3)})"
            fx.append(x)
            fy.append(y)
            ax.scatter(x, y)
        ...
        if plot_pspace:
            ...
            ax.scatter(qsd, rmax, color = c, s = 70)
