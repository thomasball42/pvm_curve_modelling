...
def generate_tasks(RUNS):
    ...
    for run_name, run_params in RUNS.items():
        ...
        varK_schedule_space = run_params["kwargs"].get("K_SCHEDULE_SPACE", [None])
        varK_strength_space = run_params["kwargs"].get("K_STRENGTH_SPACE", [None])
                    ...
                    for n0 in n0_space:
                        ...
                        for qrev in qrev_iterator:
                            for Sa in sa_iterator:
                                for b in b_space:
                                    for varK_schedule in varK_schedule_space:
                                        for varK_strength in varK_strength_space:
                                            yield (run_name, run_params, qsd, qrev, rmax, Sa, n0, b,
                                                   year_threshold, varK_schedule, varK_strength)
